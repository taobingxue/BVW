...
public class PSMoveExample : MonoBehaviour {
	...
	private static PSMoveExample instance;
	private Actions mAction;
	public string ipAddress = "128.2.237.237";
	public string port = "7899";
		
	public GameObject gem, handle,gem1, handle1,anchorL,anchorR;
	
	public bool isMirror = true,isDebug = false;
	...
	public static PSMoveExample getInstance(){
		return instance;
	}
	...
	public Transform getControllerTransform(int controllerNum){
	...
	void FixedUpdate () {
		...
		if(PSMoveInput.IsConnected && PSMoveInput.MoveControllers[0].Connected) {
			...
			if(isMirror) {
				...
				anchorR.transform.eulerAngles = handle.transform.eulerAngles;
			...
			if(moveData.ValueT > 200){
				actionChecker = (int)Actions.MOVER;
				MotionController.getInstance().setHandPosition(handle.transform.position);
			}
		}
		...
		if(PSMoveInput.IsConnected && PSMoveInput.MoveControllers[1].Connected) {
			...
			handlePos = moveData.HandlePosition;
			if(isMirror) {
				gem1.transform.localPosition = gemPos;
				handle1.transform.localPosition = handlePos;
				handle1.transform.localRotation = Quaternion.Euler(moveData.Orientation);
			}
			else {
				gemPos.z = -gemPos.z + zOffset;
				handlePos.z = -handlePos.z + zOffset;
				gem1.transform.localPosition = gemPos;
				handle1.transform.localPosition = handlePos;
				handle1.transform.localRotation = Quaternion.LookRotation(gemPos - handlePos);
				handle1.transform.Rotate(new Vector3(0,0,moveData.Orientation.z));
				anchorL.transform.eulerAngles = handle1.transform.eulerAngles;
			...
			if(moveData.ValueT > 200){
				actionChecker = (actionChecker == (int)Actions.MOVER) ? (int)Actions.ROTATE : (int)Actions.MOVEL;
				MotionController.getInstance().setHandPosition(handle1.transform.position);
			}
		}
		MotionController.getInstance ().setAction((Actions)actionChecker);
	}
